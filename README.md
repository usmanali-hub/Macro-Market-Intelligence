...
The charts below are committed as SVG so a recruiter can see the analytical output directly on GitHub without opening source code. GitHub Actions regenerates and validates the charts on every push/PR and stores the generated versions as workflow artifacts; CI does not commit back to `main`.
